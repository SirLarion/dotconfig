...
describe('user.adminAction.updateEmailAddress', async () => {
  ...
  const setup = async ({ isSuperAdmin, isScimEnabled }: ISetupConfig = {}) => {
    ...
    const admin = await insertMockUser(withCombinedRolesAndOrgId([ADMIN], organization._id));
    const superAdmin = await insertMockUser(withCombinedRolesAndOrgId([SUPER_ADMIN], 'osiehrwoieh'));
