...
describe('v274', () => {
  ...
  const setup = async () => {
    ...
    await Promise.all(
      TEST_INDUSTRIES.flatMap(async ({ orgs }) => {
        orgs.map(
          async orgId =>
  ...
  it('adds industryId to existing organizations based on the mapping in the data', async () => {
    ...
    await Promise.all(
      TEST_INDUSTRIES.flatMap(({ _id, orgs }) => {
        orgs.map(
          orgId =>
            new Promise((resolve, reject) => ctx.handlers.collection.organization.get(
                ctx,
                {
                  id: orgId,
                }
              ).then(({ industryId }) => resolve(expect(industryId).eql(_id)));
