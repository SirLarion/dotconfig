...
const PARSED_QUEST_STATES: Partial<
  Record<QuestState, 'succeeded' | 'failed' | 'missed'>
> = {
...
export const getUserActivityEventInfo = (
  ...
  // Handle rereported quests separately as the way they're displayed
  // depends on the state of the quest
  if (event.event === EAnalyticsEvent.engine_quest_rereported) {
    const questState = getParsedQuestState(event.quest?.state);
