...
export interface ITimeseriesGraphCardProps extends IAnalyticsCardProps {
...
export const TimeseriesGraphCard: TAnalyticsComponent<
  ITimeseriesGraphCardProps
> = ({ label, queryDocument, queryVariables, graphColor, ...restProps }) => {
  const [measureRef, { width }] = useMeasure();
  const theme = useTheme() as TTheme;
  const { data } = useAnalyticsQuery(queryDocument, queryVariables);

  const { base, dimmed } = colorLiteralToPalette(graphColor, theme);

  const { series, orgLatest, orgDelta, industryLatest } =
    extractSeriesProperties(data, [base, dimmed]);

  return (
    <StyledTimeseriesGraphCard ref={measureRef} {...restProps}>
      <div>
