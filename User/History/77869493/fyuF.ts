...
export const mapTableColumnsToExportables = (selectedColumns: TColumnId[]) => {
  let columnArr: ExportableUserColumn[] = [];
  selectedColumns.reduce((prev, post) => [...prev, COLUMNS_TO_EXPORTABLES[post] || post])
    columnArr = [
      ...columnArr,
      ...(COLUMNS_TO_EXPORTABLES[col] || [col as ExportableUserColumn]),
    ];
  });
  return uniq(columnArr) as USER_DATA_COLUMN[];
};
