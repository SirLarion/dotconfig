...
import { insertMockOrganization, withOrganizationOverrides } from '@server/domains/lib/testMockCreators/mockOrganization';
import { withOrganizationId } from '@server/domains/lib/testMockCreators/mockUtil';
import { withOrganization } from '@server/domains/lib/testMockCreators/mockUser';
    ...
    const orgs = await Promise.all(
      TEST_INDUSTRIES.map(async ({ orgs, ...industry }) => {
        ...
        return await Promise.all(
          orgs.map(orgId => insertMockOrganization(withOrganizationOverrides({ _id: orgId}))
        );
      })
    ).then(flatten);
