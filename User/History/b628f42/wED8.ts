...
const INVALID_EMAILS = [
  ...
  { address: 'testifoxhunt.com', error: 'Domain has to exist' },
  ...
  describe('Email', () => {
    ...
    it('should pass valid email addresses', () => {
      validationRules.forEach(rule =>
        VALID_EMAILS.forEach(email =>
          expect(rule.predicate(email)).toEqual(true)
        )
      );
    });
    ...
    it('should block invalid email addresses with the correct error message', () => {
      INVALID_EMAILS.forEach(({ address, error }) => {
        ...
        validationRules.forEach(rule => {
          if (!predicateFailed && !rule.predicate(address)) {
            expect(rule.message).toEqual(error);
