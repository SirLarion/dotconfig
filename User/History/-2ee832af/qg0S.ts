...
type TRecommendationsEdges = GetTopRecommendations_cube_edges;
type TRecommendationsNode = GetTopRecommendationsQuery['cube']['edges'];
type TQueryVariables = GetTopRecommendationsVariables;

const mapResultsToRecommendations = (
  data?: TQueryResults
): IRecommendationDatum[] =>
  R.pipe(
    R.pathOr([] as TRecommendationsEdges[], ['cube', 'edges']),
    R.map(edge => {
      const node: TRecommendationsNode = R.propOr({}, 'node', edge);
...
export const usePrioritizedRecommendations = () => {
  ...
  const { data, loading } = useGetTopRecommendationsQuery({
    variables: {
      orgId,
      date,
    },
