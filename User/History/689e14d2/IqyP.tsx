...
const ExportManyConfirmation: FC<{ loading: boolean }> = ({ loading }) => {
  if (loading) {
  ...
  return (
    <div>
      ...
      <CloseButton onClick={onActionFinish}>{BUTTON_INTL.close}</CloseButton>
    </div>
  );
};
    ...
    <StyledExportUserCSV {...restProps}>
      <Content>
        ...
        {confirmed && isManyUsers ? (
          <ExportManyConfirmation loading={loading} />
        ) : (
          <>
            <Description>
