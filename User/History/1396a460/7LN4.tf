terraform {
  backend "gcs" {
    bucket = "terraform-state-hoxhunt-com"
...
locals {
  ...
  internal_domain                         = "hoxinternal.com"
  internal_cert_validation_subdomain      = "acme"
  mail_delivery_domain                    = "unicornmx.com"
  mail_delivery_cert_validation_subdomain = "acme"
  env                                     = "master"
  ...
  iam_project_data_scientists = [
    "robin.staudinger@hoxhunt.com",
    "joonas.maanonen@hoxhunt.com",
    "markus.toivonen@hoxhunt.com",
    "max.estlander@hoxhunt.com",
    "essi.tallgren@hoxhunt.com",
    "lauri.ranta-nilkku@hoxhunt.com",
  ]

  iam_project_product_analysts = [
    "timo.herttua@hoxhunt.com",
    "miio.taarna@hoxhunt.com",
    "jonas.nabb@hoxhunt.com",
    "kosti.lepojarvi@hoxhunt.com",
    "risto.ilomaki@hoxhunt.com",
  ]

  // See the associated custom role \"bigquery_job_reviewer\" in iam-module before adding new members
  iam_bigquery_job_reviewers = [
    "timo.herttua@hoxhunt.com",
    "robin.staudinger@hoxhunt.com",
  ]
  ...
  iam_trace_exporter_service_accounts = [
    "serviceAccount:export-0000007a368e2466-2136@gcp-sa-cloud-trace.iam.gserviceaccount.com"
  ]

  iam_project_developers = []

  github_dataset_sync_service_account_email = "service-524901295206@gcp-sa-bigquerydatatransfer.iam.gserviceaccount.com" # hoxhunt-cto project

  dns_google_site_verification_token = "google-site-verification=IfVEYAWHnCdeUrl86AaLBD5LUd1gD4LKjb3RgRJ9sII"

  monitoring_alert_email = "notifications-sre-master@hoxhunt.com"
  ...
  gke_cluster_name                                = "primary-eu-1"
  gke_primary_node_pool_machine_type              = "n1-standard-4"
  ...
  gke_zookeeper_node_pool_machine_type            = "n1-highcpu-8"
  ...
  cloud_composer_storage_bucket_name = "europe-west1-cloud-composer-52136528-bucket"
  ...
  analytics_postgres_replica_machine_type              = "db-custom-8-53248" # 8 CPU <-> 52 GB RAM max, taken from GCP calculator
  ...
  mongo_atlas_ip_range = "172.23.0.0/16" # This is read from a separate terraform using remote state, so be very careful when planning to modify!

  peered_networks = {
    mongo_atlas = {
      name = "mongo-atlas"
      // The below values come from the outputs of
      // infrastructure/env/master/terraform_mongo_atlas/main.tf or alternatively from
      // https://cloud.mongodb.com/v2/60f7cb82ad70a332464cdd95#security/network/peering
      external_project_id = "p-ab1eogcrfwwklxqwckzq1duw"
      external_vpc_name   = "nt-60f7cb8694b52810f002f2ac-t7ol78fj"
    }
  }
...
module "iam" {
  ...
  project_bigquery_job_reviewer                           = local.iam_bigquery_job_reviewers
  ...
  github_dataset_sync_service_account_email               = local.github_dataset_sync_service_account_email
...
module "gke_private_cluster" {
  ...
  nat_reserved_ip_addresses_count = 2
  nat_active_ip_addresses_count   = 2
  nat_min_ports_per_vm            = 2048 // supports 32 nodes per IP
...
module "cloud_functions" {
  ...
  cloudflare_logs_storage_bucket_name                       = module.gcs.cloudflare_logs_storage_bucket_name
  application_storage_bucket_name                           = module.gcs.application_storage_bucket_name
  git_translations_repository                               = "hoxapp-i18n-translations"
...
module "dataflow" {
  ...
  analytics_ingest_max_workers                            = 16
  analytics_ingest_machine_type                           = "n1-standard-4"
  ...
  analytics_processing_dead_letter_queue_topic            = module.pubsub.analytics_processing_dead_letter_queue_topic
  analytics_cubes_ingest_pubsub_topic                     = module.pubsub.analytics_cubes_ingest_pubsub_topic
  threat_dump_parsing_max_workers                         = 4
  ...
  analytics_ingest_clickhouse_cubes_source_pubsub_subscription = module.pubsub.analytics_ingest_clickhouse_cubes_source_subscription
  analytics_ingest_clickhouse_dead_letter_queue_pubsub_topic   = module.pubsub.analytics_ingest_clickhouse_event_dead_letter_queue_topic
  analytics_ingest_clickhouse_connection_url_secret_version    = "${module.secret_manager.analytics_ingest_clickhouse_connection_url_secret_id}/versions/1"
}
