...
import { USER_GAME_MODE } from '../../../graphql/__generated__/globalTypes';
import { CurrentUserOrganizationDefaultSettingsQuery_currentUser_organization_domains as IDomain } from '../components/UserImportTable/graphql/__generated__/CurrentUserOrganizationDefaultSettings.generated';
...
export const emailsDomainIsAllowed = (email: string, domains?: IDomain[]) => {
  const [, emailDomain] = email?.split('@');
  return domains?.some(domain => domain.name === emailDomain);
};

export const getAllowedQuestLanguages = (
  email: string,
  domains?: IDomain[]
) => {
  const [, emailDomain] = email?.split('@');
...
const validateEmail = (value: unknown, allowedDomains?: IDomain[]) => {
  if (!isString(value)) {
