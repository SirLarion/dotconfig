...
import { GetQuestTemplateListDocument } from '../QuestTemplateList/__generated__/GetQuestTemplatesQuery.generated';
...
const onSubmit = (
  ...
  return mutation({
    ...
    update: async (store, result) => {
      if (isCreateMode) {
        const data = store.readQuery({ query: GetQuestTemplateListDocument });
        ...
        store.writeQuery({ query: GetQuestTemplateListDocument, data });
      }
    },
  })
    .then(response => {
