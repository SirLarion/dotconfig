...
import { IOnboardingInvitationPayload } from '../lib/sendOnboardingInvitationToAdmins.models';
...
import { withAdminRole } from '@server/domains/lib/contextWith';
...
const setup = async () => {
  ...
  const ctx = await R.pipe(
    withAppConfig({
      ...
      // organizationOnboardingWelcomeUrl: 'http://'
    }),
...
describe('admin.organizationOnboarding.sendOnboardingInvitationToAdmins', () => {
  ...
  it('should build an email that contains text and metadata according to spec', async () => {
    const { ctx, org, stubSend } = await setup();
    const admin = insertMockUser(withCombinedRolesAndOrgId([ADMIN], org._id));
