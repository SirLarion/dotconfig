...
import { withMockTaskRunnerIdentity } from '@server/domains/lib/testUtilIntegration';
import { EServerEvent } from '@server/lib/analyticEvents';
import { errorFailedPrecondition } from '@server/util/error';

import { removeOnboardingStartedAtValue } from './lib/launchOrganization.lib';
...
const launchOrganization: TLaunchOrganizationHandlerConfig = {
  ...
  async handler(ctx, { organizationId }) {
    ...
    const elevatedCtx = await withMockTaskRunnerIdentity(ctx);

    await removeOnboardingStartedAtValue(elevatedCtx, organizationId);
