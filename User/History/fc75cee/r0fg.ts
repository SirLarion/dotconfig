...
export const CSV_FILENAME = 'hoxhunt-user-data.csv';
...
const createUserListCSVFile: TCreateUserListCSVFileHandlerConfig = {
  ...
  async handler(ctx, payload) {
    ...
    const bucketName = await ctx.getConfig('csvTemporaryStorageBucket');
    ...
    return ctx.handlers.integration.googleCloudStorage.getSignedUrl(ctx, {
      resourceUrl,
      options: {
