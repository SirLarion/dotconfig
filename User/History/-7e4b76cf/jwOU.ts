...
describe('collection.user.patch', () => {
  ...
  sinon.stub(mongo, 'patch').callsFake((ctx, args) => args as any);

  const setup = async ({ isAdminCtx = false }: TSetupOverrides = {}) => {
    ...
    const admin = await insertMockUser(withOrganizationId(org._id));

    const ctx = await R.pipe(
      isAdminCtx ? withUserIdentity(admin, [ADMIN]) : withTaskRunnerUserRole,
      createIntegrationCtx
    )(null);

    return { ctx, org, user, stubPatch };
  };
  ...
  it(`should allow admins to patch email addresses`, async () => {
    const { ctx, user, org } = await setup({ isAdminCtx: true });
    ...
    const patchedUser = await ctx.handlers.collection.user.patch(ctx, {
      ...
      data: {
        emails: [{ address: 'ulrich.user@big.tings' }],
      },
    });
    expect(patchedUser.emails[0].address).eql('ulrich.user@big.tings');
  });
  ...
  it('should throw if attempting to set an email address with an empty local part', async () => {
    const { ctx, user } = await setup();
  ...
  it(`should throw if attempting to set an email address that does not exist in the org's domains`, async () => {
    const { ctx, user } = await setup();
    ...
    await expect(
      ctx.handlers.collection.user.patch(ctx, {
        id: user._id,
        data: {
          emails: [{ address: 'fella.fellerson@small.stuff' }],
        },
      })
    ).rejectedWith('Email domain small.stuff not configured in organization');
  });
