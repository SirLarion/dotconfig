...
import { TSelectedUserDataColumn } from '@server/domains/admin/userManagement/lib/createUserListCSVFile.models';
import { INewUser } from '@server/domains/collection/user/lib/create.models';
import {
  IOnboardUserPayload,
  ONBOARDING_USER_FIELDS,
} from '@server/domains/game/user/lib/onboardUser.models';
...
import {
  ...
  fetchCursorHead,
  mapCursor,
} from '@server/domains/lib/collectionFp';
...
import {
  getSingleOrganizationByEmailDomains,
  getUniqueDomains,
  getUserSearchPayload,
  TUserActionScope,
} from './utils';

/*
 * Used by generateUserCSVFile
 * Maximum amount of users that can be exported without creating
 * a task.
 * */
const MAX_USERS_FOR_FAST_EXPORT = 5000;
...
const getUsersForUserAction = async (
  ...
  const searchParams = getUserSearchPayload(args);
...
export const updateUserEmailAddress = authenticatedResolver(
  (
    root,
    args: { organizationId: string; userId: string; emailAddress: string },
    ctx
  ) => ctx.handlers.user.adminAction.updateEmailAddress(ctx, args)
);
...
export const generateUserCSVFile = authenticatedResolver(
  async (
    root,
    {
      selectedColumns,
      ...args
    }: TUserActionScope & { selectedColumns: TSelectedUserDataColumn[] },
    ctx
  ) => {
    ...
    if (userCount <= MAX_USERS_FOR_FAST_EXPORT) {
      ...
      return {
        taskGroup: await defer(
          ctx,
          ctx.handlers.admin.userManagement.runLargeUserListCsvCreationFlow,
          { ...payload, userId: ctx.identity.getEffectiveId() }
        ),
      };
    }
  }
...
export const updateOnboardingEligibilityForUsers = authenticatedResolver<
  unknown,
  { userIds: string[] }
>(async (root, { userIds: userIdParams }, ctx): Promise<IUser[]> => {
  const users = await ctx.handlers.collection.user
    .find(ctx, {
      params: {
        selector: { _id: { $in: userIdParams } },
        options: { fields: ONBOARDING_USER_FIELDS },
      },
    })
    .then(fetchCursor);
  ...
  return Promise.all(
    users.map(user =>
      ctx.handlers.game.user
        .updateOnboardingEligibility(ctx, { user })
        .then(u => ctx.handlers.collection.user.get(ctx, { id: u.user._id }))
    )
  );
});
