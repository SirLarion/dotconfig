...
import { IUpsertAnalyticsExportConfigPayload } from '@server/domains/admin/analyticsExport/lib/upsertAnalyticsExportConfig.models';
...
import { createIcuMessage } from '@server/domains/intl/translations/lib/icuMessage.lib';
import { EXTERNAL_ORGANIZATION_INTL_PREFIX } from '@server/domains/intl/translations/lib/shared.lib';
...
export const updateOrganizationComplianceNotification = authenticatedResolver(
  async (
    root,
    {
      organizationId,
    ...
    const newIcuMessage = createIcuMessage(
      `${EXTERNAL_ORGANIZATION_INTL_PREFIX}.${organizationId}.complianceNotification`,
      complianceNotification.content,
      'Message shown to users of a specific organization after reporting or failing a simulation. Message might be in markdown, so please respect any formatting like ##Headers, linebreaks or *bold* text'
    );

    await ctx.handlers.intl.translations.syncTranslations(ctx, {
      messages: [newIcuMessage],
    });
...
export const upsertOrganizationAnalyticsExportSetting = authenticatedResolver(
  async (root, args: IUpsertAnalyticsExportConfigPayload, ctx) => {
    try {
      const data =
        await ctx.handlers.admin.analyticsExport.upsertAnalyticsExportConfig(
          ctx,
          {
            organizationId: args.organizationId,
            config: args.config,
      ...
      return {
        errors: [createMutationError(err)],
      };
    }
  }
);
...
export const removeFeatureFromOrganization = authenticatedResolver(
  (root, args: { id: string; feature: string }, ctx) =>
    ctx.handlers.collection.organization
      .removeFeatureFromOrganization(ctx, {
        id: args.id,
        feature: args.feature,
      })
      .then(() =>
        ctx.handlers.collection.organization.get(ctx, { id: args.id })
      )
);
...
export const setOrganizationIndustry = authenticatedResolver(
  async (
    root,
    {
      organizationId,
      industryId,
    }: { organizationId: string; industryId: string },
    ctx
  ) => {
    const result = await ctx.handlers.admin.industries.setOrganizationIndustry(
      ctx,
...
export const toggleOrganizationOnboarding = authenticatedResolver(
  async (root, args: { organizationId: string; onboarding: boolean }, ctx) => {
    if (args.onboarding) {
      return startOrganizationOnboarding(root, args, ctx);
    }

    return organizationLaunchTraining(root, args, ctx);
  }
);
...
export const updateOrganizationThreatUploadBackupEmail = authenticatedResolver(
  async (root, { organizationId, backupEmail }, ctx) => {
    const updatedOrganization =
      await ctx.handlers.collection.organization.patch(ctx, {
        id: organizationId,
        data: {
          threatSettings: {
            threatUploadBackupEmail: backupEmail || null,
          },
        },
      });
...
export const updateOrganizationTeamsBotSettings = authenticatedResolver(
  async (root, { organizationId, teamsBotSettings }, ctx) =>
    ctx.handlers.collection.organization.patch(ctx, {
      id: organizationId,
      data: {
        teamsBotSettings,
