...
export const setOrganizationGmailIntegration = authenticatedResolver(
  async (
    root,
    {
      organizationId,
      useGmailDeliveryApi,
    }: { organizationId: string; useGmailDeliveryApi: boolean },
    ctx
  ) => {
    // If API is taken into use, allow it only if there is connectivity to it
    if (useGmailDeliveryApi) {
      const userId = ctx.identity.getId();
      const user = await ctx.handlers.collection.user.get(ctx, {
        id: userId,
        params: {
          selector: { organizationId: ctx.identity.getOrganizationId() },
        },
      });

      // Throws error if Gmail integration is not set up properly
