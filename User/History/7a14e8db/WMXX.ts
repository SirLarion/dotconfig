...
export const updateOrganizationDomains = authenticatedResolver(
  async (
    root,
    { organizationId, domains }: { organizationId: string; domains: IDomain[] },
    ctx
  ) => {
    try {
      ...
      // Disallow removing domains with this resolver
      if (domains.length < organization.domains.length) {
        throw errorInvalidArgument(
          ctx,
          'Cannot remove domains with updateOrganizationDomains. Did you mean removeOrganizationDomain?',
          withOrganizationId(organization._id),
          withUserId(ctx.identity.getId())
        );
      }

      const patchedOrganization =
        await ctx.handlers.collection.organization.patch(
...
export const removeOrganizationDomain = authenticatedResolver(
  async (root, args: { domainName: string }, ctx) => {
    try {
      const organization =
        await ctx.handlers.collection.organization.removeDomain(ctx, args);
      return {
        data: organization,
        errors: [],
      };
...
export const updateOrganizationSecurityTeamName = authenticatedResolver(
  async (root, { organizationId, securityTeamName }, ctx) => {
