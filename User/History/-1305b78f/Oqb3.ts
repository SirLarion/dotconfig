...
const setup = async () => {
  ...
  const adminCtx = await R.pipe(
    withUserIdentity(admin),
    createIntegrationCtx
  )(null);

  const superCtx = await R.pipe(
  ...
  return { org, otherOrg, ctx: adminCtx, superCtx };
};
...
describe('organization.domain.remove', () => {
  ...
  it('should not allow an admin to remove the domain of another org', async () => {
    const { otherOrg, ctx } = await setup();

    await expect(ctx.handlers.organization.domain.remove(ctx, {
