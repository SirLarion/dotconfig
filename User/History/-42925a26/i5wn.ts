...
describe.only('admin.technicalTesting.findTestTemplates', () => {
  ...
  it('should return all technical test templates', async () => {
    ...
    const f{ nodes, totalCount }= await ctx.handlers.admin.technicalTesting.findTestTemplates(
      ctx,
      {
    ...
    expect(totalCount).eql(4);
    expoect(nodes.map(node => node.tag).sort()).eql([
      'hox.quest.test.active',
      'hox.quest.test.not.active',
    ])
    const findQuestTemplatesResult = awawait ctx.handlers.collection.questTemplates.find(
      ctx
    ).then(fetchCursor);
    e
      });
