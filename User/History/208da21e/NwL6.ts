...
import { TSelectedUserDataColumn } from '../lib/createUserListCSVFile.models';
...
const FROZEN_DATE = new Date('2022-02-22');
const FROZEN_ISO = FROZEN_DATE.toISOString();
const MOCK_EXPECTED_RESOURCE_URL = `gs://${MOCK_BUCKET_NAME}/${MOCK_ORG_ID}/${MOCK_USER_ID}_${FROZEN_ISO}.csv`;

const testColumns: TSelectedUserDataColumn[] = [
  'firstName',
  'lastName',
  'email',
  'successRate',
...
const setup = async () => {
  ...
  const ctx = await R.pipe(
    withAppConfig({
      csvTemporaryStorageBucket: MOCK_BUCKET_NAME,
      googleStorageConfig: {
        projectId: 'bingbong',
      },
...
describe('admin.userManagement.createUserListCSVFile', () => {
  ...
  it('should create a CSV file and upload to GCS', async () => {
    ...
    const expectedUploadMeta = {
      bucketName: MOCK_BUCKET_NAME,
      remoteFilePath: `${MOCK_ORG_ID}/${MOCK_USER_ID}_${FROZEN_ISO}.csv`,
    ...
    const { fileContents, ...restArgs } = stubUpload.firstCall.lastArg;

    expect(fileContents).startsWith('firstName\n');
    expectedContent.forEach(content => expect(fileContents).contains(content));

    expect(restArgs).eql(expectedUploadMeta);
  });
