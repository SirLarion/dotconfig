...
const createLoginUrls = (ctx: TStartContext) => (admins: IUser[]) =>
  ctx.getConfig('defaultLoginRedirectUrl').then((defaultRedirectUrl: string) =>
    admins.map(({ _id: userId, organizationId }: IUser) =>
      ctx.handlers.auth.jwt
        .createLoginUrl(ctx, {
          userId,
...
export const sendOnboardingInvitationToOrgAdmins = async (
  ...
  return R.pipe(createLoginUrls(ctx))(admins);
};
