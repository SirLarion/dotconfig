...
export const ERROR_DUPLICATE_DOMAINS = 'Updating organization domains failed: duplicate domain names.',
  ...
  if (data.domains) {
    if (
      ...
      throw errorAlreadyExists(
        ...
        withOrganizationId(data._id),
        withOrganizationDomains(data.domains.map(({ name }) => name))
      );
    }
  }
};
...
export const sanitizeData = <T extends DeepPartial<IOrganization>>(
  ...
  validateDomainsPayload(ctx, data);
