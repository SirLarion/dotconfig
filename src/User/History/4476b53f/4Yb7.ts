...
import { insertMockQuest, withOrigin } from '@server/domains/lib/testMockCreators/mockQuest';
...
const setup = async () => {
  ...
  insertMockQuest(withOrigin(EQuestOrigin.TECHNICAL_TEST))
  ...
  return { ctx, user, organization, taskQueue };
};
...
describe('admin.organizationOnboarding.launchOrganization', () => {
  ...
  it('should remove technical test quests', async () => {
    const { ctx, user, organization } = await setup();

    await ctx.handlers.admin.technicalTesting.sendTestQuestToUser(ctx, {'hox.quest', userId: user._id, organizationId: organization._id})
  });
});
