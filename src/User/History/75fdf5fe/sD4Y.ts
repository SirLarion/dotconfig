...
export const toActiveSearchFilter = (activeState: TActive | undefined) => {
  if (!activeState) {
    return activeState;
  }
...
export const gqlFilter = selector<QuestTemplate_filter>({
...
export const gqlSearch = selector<string | undefined>({
  key: 'training-management-filter-gql-search',
  get: ({ get }) => {
    const active = get(activeFilter);

    return toActiveSearchFilter(active);
  },
});
