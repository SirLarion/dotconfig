...
const fetchEmailDeliveryAccessToken: TFetchEmailDeliveryAccessTokenHandlerConfig =
  {
    roles: [TASK_RUNNER],
    async handler(ctx, { organizationId, userId, emailAddress: email }) {
      if (!userId && !email) {
        throw errorFailedPrecondition(
          ctx,
          "Recipient's User ID or Email address must be defined"
        );
      }
      ...
      const emailAddress = user ? user.emails[0].address : email;
