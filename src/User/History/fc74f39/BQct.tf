terraform {
  backend "gcs" {
    bucket = "terraform-state-hoxhunt-dev"
...
locals {
  ...
  internal_domain                         = "hoxinternal.dev"
  ...
  iam_trace_exporter_service_accounts = [
    "serviceAccount:export-00000021c418c9ce-8076@gcp-sa-cloud-trace.iam.gserviceaccount.com"
  ]

  iam_project_developers = [
    "axel.auvinen@hoxhunt.com",
    "samuli.lehtonen@hoxhunt.com",
    "miska.tammenpaa@hoxhunt.com",
  ]

  dns_google_site_verification_token = "google-site-verification=IfVEYAWHnCdeUrl86AaLBD5LUd1gD4LKjb3RgRJ9sII"

  monitoring_alert_email = "notifications-sre-develop@hoxhunt.com"
  ...
  gke_cluster_name                                = "primary-eu-44"
  ...
  gke_tasks_node_pool_machine_type                = "n1-standard-2"
  ...
  cloud_composer_storage_bucket_name = "europe-west1-cloud-composer-2492effe-bucket"
  ...
  // infrastructure/env/develop/terraform_mongo_atlas/main.tf
  ...
  peered_networks = {
    mongo_atlas = {
      ...
      // infrastructure/env/develop/terraform/main.tf or alternatively from
      // https://cloud.mongodb.com/v2/60a4bed2abad516a9718612d#security/network/peering
      external_project_id = "p-hdljovpcwgnwyx9fqfwxsxw0"
      external_vpc_name   = "nt-60a514a0ef1e65693602c9c4-5xi0z01r"
    }
  }
...
module "cloud_functions" {
  ...
  git_translations_branch                                   = "develop"
...
module "dataflow" {
  ...
  analytics_ingest_max_workers                            = 6
