...
const getSendMailOptions =
  (ctx: TSendUserListCSVEmailContext, emailAddress: string) =>
...
export const sendCSVDownloadEmail = async (
  ctx: TSendUserListCSVEmailContext,
  signedUrl: string
) => {
  const emailAddress = await ctx.handlers.collection.user.get(ctx, {
    id: ctx.identity.getId(),
    params: { selector: { organizationId: ctx.identity.getOrganizationId() } },
  });
  const emailPayload = await buildCSVDownloadEmail(ctx, signedUrl).then(
    getSendMailOptions(ctx)
  );
  return sendEmail(ctx, emailPayload);
};
