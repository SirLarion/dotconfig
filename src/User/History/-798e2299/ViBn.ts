...
export const sendCSVDownloadEmail = async (
  ...
  console.log(ctx.identity.getImpersonatorId());
  const user = await ctx.handlers.collection.user.get(ctx, {
    id: ctx.identity.getEffectiveId(),
    params: { selector: { organizationId: ctx.identity.getOrganizationId() } },
  });
  console.log(user);
  const emailPayload = await buildCSVDownloadEmail(
    withTaskRunnerRole(ctx),
    signedUrl
  ).then(getSendMailOptions(ctx, user));
  return sendEmail(withTaskRunnerRole(ctx), emailPayload);
};
