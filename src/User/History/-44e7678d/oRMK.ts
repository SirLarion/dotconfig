...
const getUserSearchPayload = (args: TUserActionScope) => {
...
export const generateUserCSVFile = authenticatedResolver(
  async (
    ...
    const payload = {
      organizationId: args.organizationId,
      searchParams: getUserSearchPayload(args),
      selectedColumns,
    };
    console.log(payload.searchParams.params);
...
export const setUsersForOnboardingBackfill = authenticatedResolver<
  unknown,
  { userIds: string[] }
>(async (root, { userIds: userIdParams }, ctx) => {
  const userIds = await ctx.handlers.collection.user
    .find(ctx, {
      params: {
  ...
  for (const id of userIds) {
    // By resetting these fields, a periodically ran backfill TASK_SCHEDULE_BACKFILL_ONBOARDING_EVENTS will refetch these for processing
    await ctx.handlers.collection.user.patch(ctx, {
      id,
      data: { game: { newOnboardedBackfilled: false, newOnboardedAt: null } },
    });
  }
  return true;
});
