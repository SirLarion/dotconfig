...
const update: TUpdateHandlerConfig = {
  ...
  async handler(ctx, { organizationId, domainInput }) {
    ...
    // Disallow updating domain name if there are users whose email address is
    // under the domain
    if (domain.name !== domainInput.name) {
    ...
    return getDomainOrThrow(ctx, updatedOrg, domain.name);
  },
};
