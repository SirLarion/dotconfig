...
import { IFindPayload as TPayload } from '../lib/find.models';

chai.use(chaiAsPromised);

const FROZEN_DATE = new Date();

describe('collection.industry.find', async () => {
  testACL(find, [TASK_RUNNER]);
  ...
  for (const test of happyCaseTests) {
    it(test.description, async () => {
      ...
      const result = await find.handler(ctx, test.payload);
  ...
  for (const test of sadCaseTests) {
    it(test.description, async () => {
      ...
      await expect(find.handler(ctx, test.payload)).to.be.rejectedWith(
        () => test.throws
      );
    });
  }
