...
const updateEmailAddress: TUpdateEmailAddressHandlerConfig = {
  ...
  async handler(ctx, { userId, organizationId, emailAddress }) {
    ...
    if(!any(domain => emailAddress.endsWith(domain), org.domains)) {
      throw errorInvalidArgument(ctx, `Could not update email address: the domain for ${emailAddress} does not exist.`, withUserId(ctx.identity.getId()), withOrganizationId(org._id))
    }
