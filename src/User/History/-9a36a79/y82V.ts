...
describe('user.adminAction.updateEmailAddress', async () => {
      ...
      await verifyAnalytics(ctx, analyticSpies, [
        {
          ...
          userId: result._id,
          properties: {
            domain: test.payload.emailAddress.split('@')[1],
            emailAddress: test.payload.emailAddress,
          },
        },
      ]);
  ...
  const sadCaseTests: Array<{
    ...
    {
      description: `should throw if the target org doesn't exist`,
      payload: {
        organizationId: 'orgiborgi',
        userId: MOCK_USER_ID,
        emailAddress: `janteri.mocqvist${MOCK_DOMAIN}`,
      },
      isSuperAdmin: true,
      throws: `Could not update email address: organization with ID orgiborgi not found.`,
    },
    {
      description: `should throw if updating a user email from another org as an admin`,
      payload: {
        organizationId: MOCK_ORG_ID_2,
        userId: 'trivial123',
