...
export const durationToDisplayValue = (value: number) => {
  let first = '';
  let second = '';

  if (value / DAY_IN_SEC > 1) {
    first = (value / DAY_IN_SEC).toFixed(0);
    second = (value / HOUR_IN_SEC).toFixed(0);

    return `${first}d${second}h`;
  }

  if (value / HOUR_IN_SEC > 1) {
    first = (value / HOUR_IN_SEC).toFixed(0);
    second = (value / MIN_IN_SEC).toFixed(0);

    return `${first}h${second}m`;
  }

  return `${(value / MIN_IN_SEC).toFixed(0)}m`;
};
