...
import { useUpdateUserEmailAddressMutation } from 'views/UserDetails/graphql/__generated__/UpdateUserEmailAddress.generated';
...
export const UserDetailsEdit: FC<{
  ...
  const saveUserBasicInfo = (user: UserDetailsFragment) =>
    new Promise((resolve, reject) => {
      ...
      Promise.all([mutate({ variables: getSubmittable(user, values) })])
        .then(res => resolve(res.data?.updateUser))
        .catch(reject);
    });
