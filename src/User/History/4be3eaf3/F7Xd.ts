...
export const useRiskTimeseries = (group: TGroupType, name?: string) => {
  ...
  const {
    ...
    refetch: (vars: Partial<TQueryVars>) => data,
  };
  ...
  const timeseriesData = useMemo(
    () =>
      data && !loading && !error ? mapResultsToTimeseries(data, group) : [],
    [loading, error, data, group]
  );

  const timeseriesGraphData: Serie[] = useMemo(
    () => mapTimeseriesToGraphableData(timeseriesData || [], riskType),
    [timeseriesData, riskType]
  );

  const factors = useMemo(
    () =>
      factorsData && !factorsLoading && !factorsError
