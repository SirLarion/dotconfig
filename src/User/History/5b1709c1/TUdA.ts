...
const mapColumnIdsToUserData = (
  user: IUser,
  selectedDataColumns: TSelectedUserDataColumn[]
) => selectedDataColumns.map(id => `${USER_DATA_TRANSFORMS[id](user)}`);

const getFileContents = async (
  cursor: ICursor<IUser>,
  selectedDataColumns: TSelectedUserDataColumn[]
) => {
  let csvRows = [selectedDataColumns.join(',')];
  await cursor.forEach(user => {
    const userRow = mapColumnIdsToUserData(user, selectedDataColumns).join(',');
...
export const getFileWithFilePath = async ({
  ...
  const fileContents = await getFileContents(userCursor, selectedDataColumns);
