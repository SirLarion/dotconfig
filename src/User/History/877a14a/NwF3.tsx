...
import { difference, mergeDeepRight, pipe } from 'ramda';
...
export const LineGraph: FC<ILineGraphProps> = ({
  ...
  } = useMemo(() => getSerieSpecificOverrides(series, theme), [series, theme]);
  ...
  const allLayers = useMemo(
    () =>
      [
        'grid',
        'markers',
        getAreaStylingLayer(areaFillBySerieId),
    ...
    [lineStyleBySerieId, areaFillBySerieId]
  );
  ...
  const DEFAULT_GRAPH_SETTINGS: Omit<LineSvgProps, 'data'> = useMemo(
    ...
    [plain, allLayers, theme]
  );

  const props = pipe(
    mergeDeepRight(overrides),
    mergeDeepRight(DEFAULT_GRAPH_SETTINGS)
