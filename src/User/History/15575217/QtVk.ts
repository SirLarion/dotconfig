...
import { validateUserEmails } from './shared.lib';
...
export const validatePayloadEmails = async (
  ctx: THandlerContext & IMetaContext,
  payload: IPatchPayload
) => {
  if (payload.data.emails) {
    const { organizationId } = await ctx.handlers.collection.user.get(ctx, {
      id: payload.id,
      params: {
    ...
    const emails = payload.data.emails.map(({ address }) => ({
      address: address || '',
    }));

    validateUserEmails(ctx, org, emails);
  }
};
