...
import { getRecipientAddress } from '@server/domains/integration/gmail/lib/insertMessage.lib';
...
const getDeliveryPipe = async (
  ctx: TSendContext,
  organization: Pick<IOrganization, 'delivery'>,
  mail: ISendMailOptions
) =>
  isGmailDeliveryApiAvailable(ctx, organization, mail).then(available =>
...
export const deliverEmail = async (
  ...
  const deliver = await getDeliveryPipe(ctx, organization, deliveryArgs.mail);
  return deliver(ctx, {
    userId,
    organizationId: organization._id,
