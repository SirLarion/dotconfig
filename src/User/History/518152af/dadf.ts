...
import { dailyOnboardedUserTotalRiskScoreCubeAggregateOrderByFields } from '../graphql/__generated__/globalTypes';
import { TGroupUnit } from '../models';
import { dateToISODate } from '../utils/date';

import {
  GetTopRiskCountries,
  GetTopRiskCountries_cube_edges,
  GetTopRiskCountriesVariables,
...
type TQueryResults = GetTopRiskCountries | GetTopRiskDepartments | undefined;
type TQueryVars = GetTopRiskCountriesVariables | GetTopRiskDepartmentsVariables;
type TOrder = dailyOnboardedUserTotalRiskScoreCubeAggregateOrderByFields;
type TCubeEdge =
  | GetTopRiskCountries_cube_edges
  | GetTopRiskDepartments_cube_edges;
...
const queryResultsToList = (
  ...
  R.pipe(
    R.pathOr([] as TCubeEdge[], ['cube', 'edges']),
    R.map(edge => ({
...
const getQueryForGroup = (group: TGroupType) => {
  switch (group) {
    case 'department':
      return GET_TOP_RISK_DEPARTMENTS;
    case 'country':
      return GET_TOP_RISK_COUNTRIES;
    case 'org':
      return GET_TOP_RISK_COUNTRIES;
    default:
      return GET_TOP_RISK_COUNTRIES;
