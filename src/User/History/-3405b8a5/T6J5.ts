...
export const filterVisitor =
  (schema: GraphQLSchema, fieldTypeName?: string) =>
  <TSource, TContext>(field: GraphQLField<TSource, TContext>) => {
    ...
    if (fieldTypeName && !typeMap[fieldTypeName]) {
      throw new Error(
        `Cannot use @filterable(${fieldTypeName}). Type does not exist.`
      );
    }

    const fieldType =
      typeMap[fieldTypeName] || getNullableType(nullableListType.ofType);
