...
export const filterVisitor =
  (schema: GraphQLSchema, fieldType?: GraphQLOutputType) =>
  <TSource, TContext>(field: GraphQLField<TSource, TContext>) => {
    ...
    const filterType = fieldType || getNullableType(nullableListType.ofType);
    const typeMap = schema.getTypeMap();
    const fieldKey = `${filterType}${FILTER_SUFFIX}`;

    const inputFields = constructInputFields(
      Object.values(filterType.getFields())
    );
