...
const Root = `
  ...
  type Query @external {
    ...
    # A list of threats as connection
    ...
    # List of all industries in the system
    # @filterable @paginatable @sortable
    ...
    compiledQuestTemplateEmail(emailTemplate: QuestTemplateEmailTemplateInput!, organizationIdForContext: String, languageOverride: SUPPORTED_LOCALE): Email
    ...
    # Generate download URL for Teams bot app package
    teamsAppPackageSignedUrl: String!

    # A list of workflow configs
    # @filterable @paginatable @sortable
  ...
  type Mutation @external {
    ...
    # Set markers watched
    handleMarkersViewed(questId: ID!): Quest

    # Rate a single threat, and send feedback to reporter
    rateThreat(rating: RateThreatInput!): RateThreatResult

    # Rate all threats in a single incident
    classifyIncidentsThreatsBySoc(incidentId: String!, classification: SocClassification!, customMessage: String, threatFeedbackReportedAtLimit: Date): [Threat!]!

    # Set soc classification for incident
    classifyIncidentBySoc(incidentId: String!, classification: SocClassification!): Incident!
    ...
    updateIndustry(industryId: ID!, industryInput: IndustryInput!): Industry

    # Delete Industry
    deleteIndustry(industryId: ID!): Industry
    ...
    # Patch a quest template
    patchQuestTemplate(questTemplateId: ID!, questTemplate: QuestTemplatePatchInput!): QuestTemplate
    ...
    updateOnboardingEligibilityForUsers(userIds: [ID!]!): [User!]!
    ...
    addDomain(organizationId: ID!, domainInput: DomainInput!): DomainMutationResult!

    # Remove a domain from an organization. Successful if no users with email addresses under the domain exist.
    removeDomain(organizationId: ID!, domainInput: DomainInput!): DomainMutationResult!

    # Update organization's domain
    updateDomain(organizationId: ID!, domainInput: DomainInput!): DomainMutationResult!
    ...
    setOrganizationIndustry(organizationId: ID!, industryId: ID!): Boolean
    ...
    # Create or update organization analytics export settings
    upsertOrganizationAnalyticsExportSetting(organizationId: ID!, config: OrgAnalyticsExportSettingsInput!): OrganizationExportSettingsResult!
    ...
    removeFeatureFromOrganization(id: ID!, feature: String!): Organization!

    # Adds (an existing) feature for the specified organization. Can't be used for creating new features.
    addFeatureForOrganization(id: ID!, feature: String!): Organization!
    ...
    # Toggle whether organization is onboarding or not
    toggleOrganizationOnboarding(organizationId: ID!, onboarding: Boolean!): OrganizationMutationResult!

    # Update organizations security team name
    updateOrganizationSecurityTeam(organizationId: ID!, securityTeamName: String, securityTeamEmailAddress: String): Organization

    # Update compliance notification shown to users after reporting
    updateOrganizationComplianceNotification(organizationId: ID!, complianceNotification: ComplianceNotificationInput!): Organization!

    # Update organizations threat upload backup email
    updateOrganizationThreatUploadBackupEmail(organizationId: ID!, backupEmail: String!): Organization

    # Expire a quest manually
    expireQuest(questId: ID!): Quest

    # Add threat tag for threat
    addThreatTagForThreat(threatId: ID!, threatTagId: ID!): Threat!

    # Remove threat tag from threat
    removeThreatTagFromThreat(threatId: ID! threatTagId: ID!): Threat!

    # Create training packages
    createTrainingPackage(trainingPackage: TrainingPackageInput!): TrainingPackage!

    # Update training package
    updateTrainingPackage(trainingPackageId: ID!, trainingPackage: TrainingPackageInput!): TrainingPackage

    # Act on a trainingPackages objective
    actOnTrainingPackage(trainingPackageId: ID!, userId: ID!, organizationId: ID!, action: BlockSetActionsInput): UserTrainingPackageActionResult


    # Note: flags typing to be improved soon

    # Runs the organization for users, and logs the result (does not create quests)
    # dryRunIdentifier overrides transactionId, which defaults to the same string for all dry runs. When it is not passed, separate dry runs use the same memoization (which is ok for most usage)
    dryRunSimulationEngineForOrganization(organizationId: ID!, flags: [EngineFlag!], limit: Int, dryRunIdentifier: String, usersPerMinute: Int): TaskGroup!

    # Runs the organization for users, and logs the result (does not create quests)
    dryRunSimulationEngineForUsers(userIds: [ID!]!, flags: [EngineFlag!], limit: Int): [EngineDryRunResult!]!


    # Create threat tag
    createThreatTag(name: String!): ThreatTag!

    # Delete threat tag
    removeThreatTag(id: ID!): ThreatTag!

    # Send a Slack message
    sendSlackMessage(userEmail: String, channelId: String, message: SlackMessageInput!): SlackSendMessageResponse!

    # Act on a trainingPackages objective
    restartTrainingPackage(trainingPackageId: ID!, userId: ID!, organizationId: ID!): UserTrainingPackageActionResult

    # Update organization's Teams bot settings
    updateOrganizationTeamsBotSettings(organizationId: ID!, teamsBotSettings: TeamsBotSettingsInput!): Organization!

    # Create a workflow config
    createWorkflowConfig(config: WorkflowConfigInput!): WorkflowConfig

    # Update a workflow config
    updateWorkflowConfig(id: ID!, config: WorkflowConfigInput!): WorkflowConfig

    # Remove a workflow config
    removeWorkflowConfig(id: ID!): WorkflowConfig
  }
`;
