...
const mockWelcomeUrl = 'http://localhost.welcome';
...
const setup = async () => {
  ...
  const ctx = await R.pipe(
    withAppConfig({
      ...
      organizationOnboardingWelcomeUrl: mockWelcomeUrl,
    }),
...
describe('admin.organizationOnboarding.sendOnboardingInvitationToAdmins', () => {
  ...
  it('should build and send an email with content according to spec', async () => {
    ...
    expect(mail).eql('welcome@hoxhunt.com');
    expect(mail.html).contain(mockWelcomeUrl);
    expect(mail.html).contain(`Hi, ${adminFirstName}`);
  });
