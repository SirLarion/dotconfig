...
export const NO_ADMINS_ERROR = 'No admin users exist in organization';

const buildEmail =
  (ctx: TOnboardingInvitationContext) =>
  (buildConfig: {
    context: { [key: string]: string | IUser };
...
const createLoginUrl =
  ...
  ({ _id: userId }: IUser) =>
    ctx
      .getConfig('organizationOnboardingWelcomeUrl')
      .then(welcomeUrl =>
  ...
  const emails = await Promise.all(
    admins.map((admin: IUser) =>
      R.pipe(
        ...
        R.then(buildOnboardingInvitationEmail(ctx, admin)),
