...
export const CSVColumnSelector: FC<ICSVColumnSelectorProps> = ({
  ...
  const comboComponentOnClick = (
    colId: TColumnId,
    parentId: TColumnId,
    comboIds: TColumnId[]
  ) =>
    setSelectedColumnIds(currentlySelected => {
      if (!currentlySelected.includes(colId)) {
        return all(
          (comboId: TColumnId) => currentlySelected.includes(comboId),
          without([colId], comboIds)
        )
          ? [...currentlySelected, colId, parentId]
          : [...currentlySelected, colId];
      }

      return without([colId, parentId], currentlySelected);
    });
  ...
  return (
    <StyledCSVColumnSelector {...restProps}>
      <ColumnSelect>
        {Object.values(AVAILABLE_COLUMNS).map(
            ...
            return (
              <>
                <SelectedColumnItem
                  key={id}
                  $disabled={disabled}
                  onClick={() => !disabled && columnItemOnClick(id, comboField)}
                >
                  <Checkbox
                  ...
                  comboField.map(comboId => {
                    ...
                    return (
                      <IndentedColumnItem
                        ...
                        onClick={() =>
                          !comboDisabled &&
                          comboComponentOnClick(comboId, id, comboField)
                        }
                      >
                        <Checkbox
