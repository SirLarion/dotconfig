      ...
      {analytics ? (
        CARD_GROUPS.map(({ heading, cards }, groupIndex) => (
          <CardGroup key={groupIndex} heading={heading}>
            ...
            {groupIndex === 0 && <DualMetric key="eoihroi" title="Average reporting time" latestValue={{ primary: 9300, secondary: 7100 }} valueOptions={{ displayFormat: value => }} secondaryValueLabel={INTL.yourIndustry} delta={{value: 5, sentiment: 'negative', text: 'in the last month'}} />}
          </CardGroup> 
        ))
      ) : (
