  ...
  return (
    <StyledTimeseriesGraphCard {...restProps}>
      <div>
        ...
        <GraphMeta>
          <Legend>
            <LineStyleDisplay $color={graphColor.base} $style="solid" />
            <OrgLatest>{toDisplayPercentage(latestValue, 1)}</OrgLatest>
          </Legend>
          <Legend>
            <LineStyleDisplay $color={graphColor.dimmed} $style="dashed" />
            {/* TODO: Add industry comparison data */}
            <IndustryLatest>--%</IndustryLatest>
          </Legend>
          <LegendText dimmed>
            <DeltaTag
              delta={delta}
              type={delta * deltaSentiment > 0 ? 'positive' : 'negative'}
              unitMark="%"
              renderAsPill
              small
            />
      ...
      <GraphWrapper>
        <LineGraph
          ...
          overrides={{
            ...
            tooltip: ({ point }) => (
              <Tooltip
                tooltipBoxChildren={<TooltipContent>{point.y}</TooltipContent>}
              />
            ),
          }}
          plain
