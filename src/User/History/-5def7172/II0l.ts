...
describe('organization.domain.add', () => {
  ...
  const sadCaseTests: Array<{
    ...
    {
      payload: ,
      throws: `Adding domain failed. Domain: ${EXISTING_DOMAIN.name} already exists.`,
    },
  ];

    it('should throw if trying to add a domain that already exists', async () => {
      await expect(ctx.handler.organization.domain.add(ctx, { organizationId: org._id, domainInput: EXISTING_DOMAIN })).to.be.rejectedWith(
        () => test.throws
      );
    });
});
