...
import { USER_ACTIVITY_TIMELINE_EVENT_MESSAGES as INTL } from '../../intl';
...
export const getRereportedQuestEventTitle = (questState?: QuestState) => {
  switch (questState) {
    case QuestState.QUEST_STATE_SUCCESS:
      return 'succeeded';
    case QuestState.QUEST_STATE_FAILED:
      return 'failed';
    case QuestState.QUEST_STATE_MISSED:
      return 'missed';
    default:
      return undefined;
  }
...
export const getUserActivityEventInfo = (
  ...
  if (event.event === EAnalyticsEvent.engine_quest_rereported) {
    const title = getRereportedQuestEventTitle(event.quest?.state);
    if (!title) {
      return undefined;
    }
    return {
