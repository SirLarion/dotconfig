...
export type TLineProps = Serie & {
  lineOptions?: {
    style: 'solid' | 'dashed';
    fille: 'none' | 'translucent' | 'gradient';
  };
};

export interface ILineGraphProps {
  lineProps: TLineProps[];
...
export const LineGraph: React.FC<ILineGraphProps> = ({
  ...
  const DEFAULT_GRAPH_SETTING: Omit<LineSvgProps, 'data'> = useMemo(
    () => ({
      // Graph
