...
import {
  createThreatTag,
  removeThreatTag,
} from '@server/api/graphql/resolvers/mutations/dynamicTagMutations';
import {
  createFeedbackRule,
  removeFeedbackRule,
  updateFeedbackRule,
...
import {
  addIncidentNote,
  classifyIncidentBySoc,
  classifyIncidentsThreatsBySoc,
  deleteIncidentNote,
...
import {
  ...
  deleteIndustry,
  updateIndustry,
} from '@server/api/graphql/resolvers/mutations/industryMutations';
import {
...
import {
  ...
  toggleOrganizationOnboarding,
  updateOrganization,
  updateOrganizationComplianceNotification,
  updateOrganizationIncidentPolicySettings,
  updateOrganizationIncidentSettings,
  updateOrganizationSecurityTeam,
  updateOrganizationTeamsBotSettings,
  updateOrganizationThreatUploadBackupEmail,
  upsertOrganizationAnalyticsExportSetting,
...
import {
  deleteQuest,
  dryRunSimulationEngineForOrganization,
  dryRunSimulationEngineForUsers,
  expireQuest,
  handleMarkersViewed,
...
import { sendSlackMessage } from '@server/api/graphql/resolvers/mutations/slackMutations';
import {
  addThreatTagForThreat,
  deleteThreat,
  processThreat,
  rateThreat,
  removeThreatTagFromThreat,
...
} from '@server/api/graphql/resolvers/mutations/trainingPackageMutations';
import {
...
import {
  ...
  undoSoftDeleteUser,
  unlinkPlugin,
  updateOnboardingEligibilityForUsers,
  updateUser,
...
} from '@server/api/graphql/resolvers/mutations/workflowConfigMutations';
...
import { TGenerateInstallationUrlPayload } from '@server/domains/integration/slack/lib/generateInstallationUrl.models';
...
import { constructThreatSelector } from '../../../domains/threat/search/findThreatsBySearchString';
...
import {
  ...
  removeDomain,
  updateDomain,
} from './mutations/domainMutations';
import {
...
import {
  actOnTrainingPackage,
  restartTrainingPackage,
} from './mutations/userTrainingPackages';
...
export default () => ({
  Query: {
    ...
    threatsConnection: authenticatedResolver(
      async (root, args: TExtensionArgs<{ search?: string }>, ctx, info) => {
        const params = R.pipe(
    ...
    industryConnection: authenticatedResolver(
      async (root, args: TExtensionArgs, ctx, info) => {
        const params = R.pipe(getExtensionsMongoSelector)(args);

        return createConnection(
          ctx,
          ctx.handlers.collection.industry,
          params,
          info
        );
      }
    ...
    compiledQuestTemplateEmail: authenticatedResolver(
      (
        root,
        { emailTemplate, organizationIdForContext, languageOverride },
        ctx
      ) => {
    ...
    slackInstallationUrl: authenticatedResolver(
      (root, args: TGenerateInstallationUrlPayload, ctx) =>
        ctx.handlers.integration.slack.generateInstallationUrl(ctx, args)
    ),
    trainingPackages: authenticatedResolver(
      async (root, args: TExtensionArgs, ctx) => {
        const params = getExtensionsMongoSelector(args);

        const cursor = await ctx.handlers.collection.trainingPackage.find(ctx, {
          params,
        });
    ...
    threatTags: authenticatedResolver(
      async (root, args: TExtensionArgs, ctx) => {
        const params = overrideQueryParams(getExtensionsMongoSelector(args), {
          selector: {
            categoryName: 'threat tag',
          },
        });

        const cursor = await ctx.handlers.collection.dynamicTag.find(ctx, {
          params,
        });
    ...
    workflowConfigs: authenticatedResolver(
      async (root, args: TExtensionArgs, ctx) => {
        const params = overrideQueryParams(getExtensionsMongoSelector(args));

        const cursor = await ctx.handlers.collection.workflowConfig.find(ctx, {
          params,
        });
    ...
    teamsAppPackageSignedUrl: authenticatedResolver(
      async (_root, _args: {}, ctx) => {
        const { signedUrl } =
          await ctx.handlers.integration.teams.createAppPackageDownloadUrl(
            ctx,
            {}
          );
