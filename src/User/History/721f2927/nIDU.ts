...
const isValidLocalColumnStorage = (obj: Record<string, unknown>) => {
  ...
  if (typeof customLayouts !== 'object' || !customLayouts) {
    if (selected === 'default' || selected === 'performance') {
      return true;
    }
  ...
  if (!Object.keys(customLayouts).includes(selected)) {
    return false;
  }

  console.log(Object.values(obj['customLayouts']));
...
const getFromLocalStorage = (): TLocalStorageColumnLayouts => {
  ...
  //   return DEFAULT_LAYOUT_CONFIG;
  // }
  console.log(Object.values(storedLayoutObj['customLayouts']));
