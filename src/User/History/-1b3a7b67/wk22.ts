...
describe('collection.organization.patch', () => {
  ...
  it('should throw if the payload contains domains with the same name', async () => {
    ...
    await expect(
      ctx.handlers.collection.organization.patch(ctx, payload)
    ).rejectedWith(ERROR_DUPLICATE_DOMAINS);
  });

  it('should not throw if the payload domains have no duplicates', async () => {
    const { ctx, payload } = await setup({});
    await expect(ctx.handlers.collection.organization.patch(ctx, payload))
      .fulfilled;
  });
  ...
  describe('handler', () => {
    testSanitation(
      async (ctx, data) =>
        ctx.handlers.collection.organization.patch(ctx, { id: 'tmp', data }),
      (collection, data) => {
        [
          { _id: 'tmp' },
          {
