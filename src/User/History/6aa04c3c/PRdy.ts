...
const buildOnboardingInvitationEmail = (ctx: TStartContext) => (user: IUser) =>
  ctx
...
const buildAndSendAsUser = (ctx: TStartContext) => async (user: IUser) => {
  const userCtx = withIdentityFromAuthInfo(ctx, {
    authenticationLevel: EIdentityAuthenticationLevel.NORMAL,
    user,
  });
  return R.pipe(
    buildOnboardingInvitationEmail(withTaskRunnerRole(userCtx)),
